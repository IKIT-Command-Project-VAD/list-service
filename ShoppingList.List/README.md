...
To update the database use this command from the Web project folder (replace `ShoppingList.List` with your project's name):

```powershell
dotnet ef database update -c AppDbContext -p ../ShoppingList.List.Infrastructure/ShoppingList.List.Infrastructure.csproj -s ShoppingList.List.Web.csproj
```
